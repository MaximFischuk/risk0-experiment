...
use risc0_zkvm::{
    compute_image_id, recursion::identity_p254, sha::Digestible, stark_to_snark, ExecutorEnv,
    Prover, Receipt,
};
...
impl LocalProver {
    pub fn prove(elf: &[u8], input: &[u8]) -> Result<(Vec<u8>, FixedBytes<32>, Vec<u8>)> {
        log::info!("Creating the prover.");
        let prover = risc0_zkvm::LocalProver::new("local");
        let prover_server = risc0_zkvm::get_prover_server(&risc0_zkvm::ProverOpts::default())?;

        log::info!("Writting input data to the executor.");
        let env = ExecutorEnv::builder().write_slice(input).build()?;

        log::info!("Running the executor.");
        ...
        log::info!("Compressing the receipt.");
        let composite_receipt = receipt.inner.composite()?;
        let succinct_receipt = prover_server.compress(composite_receipt)?;

        log::info!("Creating the seal.");
        let ident_receipt = identity_p254(&succinct_receipt)?;
        let seal_bytes = ident_receipt.get_seal_bytes();

        log::info!("Converting the stark to snark.");
        let snark = stark_to_snark(&seal_bytes)?; // This operation requires installed Docker
        let seal = Seal::abi_encode(snark).context("Read seal")?;

        let post_state_digest = claim
            .post
            .digest()
            .as_bytes()
            .try_into()
